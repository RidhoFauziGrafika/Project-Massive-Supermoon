...
async function editTour(req, res) {
    try {
      ...
      const existingImages = await dbExecute(
        "SELECT img_path FROM tour_images WHERE tour_id = (SELECT id FROM tours WHERE slug = ?)",
        [tourSlug]
      );
      ...
      if (req.image_path && req.image_path.length > 0) {
        ...
        for (const imgPath of req.image_path) {
          await dbExecute(
            `INSERT INTO tour_images (id_uuid, tour_id, img_path, created_at) VALUES (?, (SELECT id FROM tours WHERE slug = ?), ?, ?)`,
            [uuidv4(), tourSlug, imgPath, new Date()]
          );
        }
      }
